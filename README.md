...
> 1. 정적 행정동 토지신호는 시간 효과 위에 혼잡도 정보를 사실상 더하지 못한다 (증분 R² ≈ +0.01). <br>
...
cf. R² 0.30은 토지의 설명력이 아닌, 대부분 **시간대가 만든 설명력** (참고: 아래 ablation).
...
**Train/Test Split**
- `StratifiedGroupKFold(group = 노선_역명)` — 같은 역이 train/test 양쪽에 못 들어감 (데이터 누수 차단)
